...
//   } catch (err) {
//     res.status(500).json({ success: false, message: 'Download tracking failed.' });
//   }
// };
...
/**
 * Materials Controller
 * CRUD for study materials with Supabase storage
 */
const Material = require('../models/Material');
const { upload, getFileType, uploadToSupabase } = require('../middleware/upload');
const { createClient } = require('@supabase/supabase-js');
...
exports.getMaterials = async (req, res) => {
  try {
    ...
    // Hide file URLs for members-only if user doesn't have access
    const sanitized = materials.map(m => {
      const item = { ...m };
      if (m.accessType === 'members_only' && !hasFullAccess) {
        delete item.filePath;
...
exports.updateMaterial = async (req, res) => {
  try {
    const {
      ...
      accessType, downloadAllowed, tags, isActive
    } = req.body;
...
exports.deleteMaterial = async (req, res) => {
  try {
    ...
    // Delete from Supabase if cloud URL
    if (material.filePath && material.filePath.startsWith('http')) {
      try {
        const supabase = createClient(
          process.env.SUPABASE_URL,
